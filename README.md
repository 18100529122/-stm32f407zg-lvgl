...
- **硬件驱动 (BSP)**:
  ...
  - **ADC**: 实现 **1Msps** 高速原始采样，采用定时器硬件触发 + DMA 循环搬运。
    - **双缓冲 (Ping-Pong) 机制**: 采用 **8192** 点 (16KB) 大容量缓冲区，配合半完成 (`HalfCplt`) 与全完成 (`Cplt`) 中断，实现数据采集与抽取的并行处理，极大提升了抗系统抖动能力。
    - **内存布局**: 采样缓冲区与 FIFO 结构体均部署于 **片内 SRAM**，彻底解决了与 LCD 刷屏任务抢占 FSMC 总线导致的 DMA 停滞问题。
    - **中断级自愈**: 实现了硬件错误回调 (`HAL_ADC_ErrorCallback`)，在发生 Overrun (溢出) 时于微秒级自动清除标志位并重启采集，无需应用层干预即可保证 24/7 稳定运行。
    - **高效抽取**: 在 ISR 中实现 **200 倍硬件级抽取**（有效 $F_s = 5kHz$），采用索引跳转优化算法，极大地降低了中断 CPU 占用。
    - **相位解析 (PRPD)**: 实现了工频同步的相位映射算法，将脉冲峰值精确映射到 0-360° 相位区间，支持频次累积统计。
    - **优先级管理**: 将 ADC DMA 优先级设为 **`VERY_HIGH`**，确保在总线繁忙时采样数据的绝对优先权。
